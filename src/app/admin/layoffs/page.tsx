    ...
    <div>
      ...
      <div className="mb-4 flex gap-1 rounded-lg border border-neutral-800 bg-neutral-900 p-1 w-fit">
        {tabs.map((tab) => (
          <Link
            key={tab.value}
            href={`/admin/layoffs?status=${tab.value}${params.search ? `&search=${params.search}` : ""}`}
            ...
            {items.map((l) => (
              <tr key={l.id} className="transition hover:bg-neutral-900/50">
                ...
                <td className="px-4 py-3">
                  {l.affectedCount?.toLocaleString() ?? "—"}
                  {l.isShutdown && <span className="ml-1 text-red-400" title="Shutdown">💀</span>}
                </td>
                <td className="px-4 py-3 text-neutral-400">{l.country}</td>
                <td className="px-4 py-3">
                  <span className={`rounded-full border px-2 py-0.5 text-xs ${STATUS_COLORS[l.status]}`}>
                    {l.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-right">
                  <LayoffActions id={l.id} status={l.status} />
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={`/admin/layoffs?status=${status}&page=${p}${params.search ? `&search=${params.search}` : ""}`}
