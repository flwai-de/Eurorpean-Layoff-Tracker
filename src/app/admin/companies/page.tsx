  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/admin/companies/new" className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-neutral-900 transition hover:bg-neutral-200">+ New Company</Link>
      </div>
      <form className="mb-6">
      ...
      <div className="overflow-hidden rounded-xl border border-neutral-800">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-neutral-800">
            {items.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-neutral-500">No companies found.</td></tr>}
            {items.map((c) => (
              <tr key={c.id} className="transition hover:bg-neutral-900/50">
                ...
                <td className="px-4 py-3"><span className="rounded-full bg-neutral-800 px-2 py-0.5 text-xs">{c.companyType}</span></td>
                <td className="px-4 py-3 text-neutral-400">{c.employeeCount?.toLocaleString() ?? "—"}</td>
                <td className="px-4 py-3 text-right"><Link href={`/admin/companies/${c.id}/edit`} className="text-neutral-400 transition hover:text-white">Edit</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link key={p} href={`/admin/companies?page=${p}${params.search ? `&search=${params.search}` : ""}`}
